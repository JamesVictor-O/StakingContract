...
describe("StakingPool", function () {
  ...
  async function deployStakingPool() {
    const [owner, otherAccount, othersAccount2] = await hre.ethers.getSigners();
    const { poolToken } = await deployPoolToken();
    const StakingPool = await hre.ethers.getContractFactory("StakingPool");

    const stakingPool = await StakingPool.deploy(poolToken);

    return { poolToken, stakingPool, owner, otherAccount, othersAccount2 };
  }
  ...
  describe("Test Create Pool Function", function () {
    ...
    it("Should not create a pool if Max amount of Pool is reached", async function () {
      ...
      await expect(
        stakingPool.createPools(minAmount, maxAmount, duration)
      ).to.be.revertedWithCustomError(stakingPool, "PoolMax");
    });
    ...
    it("Should emit event when pool is created successfully", async function () {
      ...
      expect(stakingPool.createPools(minAmount, maxAmount, duration))
        .to.emit(stakingPool, "PoolCreated")
        .withArgs(owner, totalPools);
    });
  });
  describe("Test Stake to Pool function", function () {
    it("Should not stake to invalid pool", async function () {
      ...
      await expect(
        stakingPool.stakeToPool(2, amountToStake)
      ).to.revertedWithCustomError(stakingPool, "InvalidPoolId()");
    });
    ...
    it("Should not allow stake below minimum", async function () {
      ...
      const amountToStake = ethers.parseEther("0");
      await expect(
        stakingPool.stakeToPool(1, amountToStake)
      ).to.revertedWithCustomError(stakingPool, "AmountTooLow");
    });
    ...
    it("Should not stake if owner does not have enough token", async function () {
      ...
      const amountToStake = ethers.parseEther("3");
      await expect(
        stakingPool.connect(otherAccount).stakeToPool(1, amountToStake)
      ).to.revertedWithCustomError(stakingPool, "InsufficientBalance");
    });
    it("Should allow user to stake if the have enough token", async function () {
      ...
      await expect(
        ...
        .withArgs(otherAccount, amountToStake);
      expect(await poolToken.balanceOf(stakingPool)).to.equal(amountToStake);
    });
  });
  describe("Calculate Stake Reward", function () {
    it("Should not calculate for Invalid pool", async function () {
      const { poolToken, stakingPool, owner, otherAccount } =
        await deployStakingPool();
      const minAmount = 2;
      const maxAmount = 5;
      ...
      await poolToken.connect(otherAccount).approve(stakingPool, 50);
      await stakingPool.connect(otherAccount).stakeToPool(1, 3);

      await expect(
        stakingPool.calculateReward(2)
      ).to.be.revertedWithCustomError(stakingPool, "InvalidPoolId");
    });

    it("Should not Revert when some one who has not stake tries to Calculate ", async function () {
      const { poolToken, stakingPool, owner, otherAccount, othersAccount2 } =
        await deployStakingPool();
      const minAmount = 2;
      const maxAmount = 5;
      const currentTimestamp = (await hre.ethers.provider.getBlock()).timestamp;
      ...
      await expect(
        stakingPool.connect(othersAccount2).calculateReward(1)
      ).to.be.revertedWithCustomError(stakingPool, "NoStakeFound");
    });

    it("Should output expected reward", async function () {
      const { poolToken, stakingPool, owner, otherAccount, othersAccount2 } =
        await deployStakingPool();
      const minAmount = 2;
      const maxAmount = 5;
      ...
      const reward = await stakingPool.connect(otherAccount).calculateReward(1);
      expect(reward).to.greaterThan(0);
    });
  });
  describe("Test Withdrawal function", async function(){
    it("Should revert with 'You did not stake in this Pool' for a staker with no stake", async function () {
      const { poolToken, stakingPool, owner, otherAccount } =
        await deployStakingPool();
      const minAmount = 2;
      const maxAmount = 5;
      ...
      await expect(stakingPool.connect(otherAccount).withdrawReward(1)).to.be.revertedWith("You did not stake in this Pool")
    });
    it("Should withdraw rewards for a valid staker", async function () {
      const { poolToken, stakingPool, owner, otherAccount } =
        await deployStakingPool();
      const minAmount = 2;
      const maxAmount = 40;
      ...
      await stakingPool.connect(otherAccount).stakeToPool(1,10)
      

      await poolToken.transfer(stakingPool,100)
      const initialBalance=await poolToken.balanceOf(otherAccount);
      // const currentBalance= await stakingPool.connect(otherAccount).withdrawReward(1)
      const amountToWithdraw=await stakingPool.connect(otherAccount).calculateReward(1)
